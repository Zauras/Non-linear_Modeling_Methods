...
comm = MPI.COMM_WORLD
rank = comm.Get_rank()
CPUcount = comm.Get_size()
host = MPI.Get_processor_name()

if (rank == 0):
    file = open('Data.txt')
    dataList = [file.readline().strip().split(' ') for cpu in range(CPUcount)]
    if(len(dataList) != CPUcount): print ('!!! Warning !!! Faile maziau duomenu nei uzsakyta CPU!')

    for CPU in range(1, CPUcount): # Paskirstom rez(zingsnelius po workerCPUs)
        data = dataList[CPU] # isiunciam kiekvienam CPU po innerList[a, c, d]
        comm.send( data, dest=CPU )
        print ('Issiunciau data i CPU:', CPU)
    data = dataList[0] # Master CPU duodam duomenis (jis irgi skaiciuos)

elif (rank != 0): # for rank in (1, CPUcount+1): Arba kitaip visiems darbiniams CPU
...
def Testas_1(Tau, h):
    ...
    for k in range(0, len(errorList)-1):
        ...
        if (70.0 < error):
            test1_Result =  'Paklaida: ' + str("%.2f"%error) + ' kartu Testas 1 - SUCCESS'
            resultList.append(test1_Result)
        else:
            test1_Result =  'Paklaida:' + str("%.2f"%error) + 'kartu  Testas 1 - FAIL'
            resultList.append(test1_Result)
    return resultList
...
def Testas_2 (Tau, h):
    ...
    for k in range(0, len(errorList)-1):
        ...
        if (9500 < error and error < 12000):
            test2_Result = 'Paklaida: ' + str("%.2f"%error) + ' kartu Testas 1 - SUCCESS'
            resultList.append(test2_Result)
        else:
            test2_Result = 'Paklaida:' + str("%.2f"%error) + 'kartu  Testas 1 - FAIL'
            resultList.append(test2_Result)
    return resultList
...
def Testas_3():
    ...
    if(maxError < delta): test3_Result = 'Paklaida:' + str(maxError) + ' Testas 3 - SUCCESS'
    else: test3_Result = 'Paklaida: ' + str(maxError) +' Testas 3 - FAIL'
    return test3_Result
...
###################### ALGORITMAS ############################
def Algoritmas(t, T):
    test1 = Testas_1(Tau, h)
    test2 = Testas_2(Tau, h)
    test3 = Testas_3()
    #print ('================= Atlikimas =================')
    ...
    while (t <= T):
        ...
        iterCount = 0 # apsauga nuo while(true)s

        #Testuojam uj^ : Paklaida = max|ujNew - ujOld)|
        tikslu = False
        while (not tikslu):
            tikslu, maxError = uNew_Tikslinimas(ujSt_Old, ujSt_New)
            if (maxError != False):
            ...
            ujSt_New = Thomas_Algoritmas(C, Fj)
            if iterCount > 20:
                print('CPU:', rank, 'Konvergacija - FAIL')
                break
        t = t+Tau
        uj = u_tikslus_List(t)
        ujSt_Old = ujSt_New
    maxErrorOfAlg = Get_MaxValue_List(maxErrorList)
    return test1, test2, test3, maxErrorOfAlg
...
info = 'a = ' + str(a)+ ' | c = ' + str(c)+ ' | d = '+ str(d)
test1, test2, test3, maxErrorOfAlg = Algoritmas(t, T)
data = info, test1, test2, test3, maxErrorOfAlg
if(rank != 0):
    comm.send( data, dest=0 )

# Griztam i Master CPU
if(rank == 0):
    dataList = [data]
    for CPU in range(1, CPUcount):
        data = comm.recv(source=CPU) # wait to get data from WORKER
        print ('Gavau data is CPU:', CPU)
        dataList.append(data)

    print ('############# SPRENDIMAS ####################')
    print ('N =', N, '| h =', h, '| Tau =', Tau)
    print ('Test x =', test_x, ' Test t = ', test_t)

    for CPU in range(len(dataList)):
        print('@@@@@@@@@@@@@@@ CPU:', CPU, '@@@@@@@@@@@@@@@@@')
        print(' ######## Info #######')
        print(dataList[CPU][0])
        print(' ######## Test 1 #######')
        for testResult in dataList[CPU][1]:
            print(testResult)
        print(' ######## Test 2 #######')
        for testResult in dataList[CPU][2]:
            print(testResult)
        print(' ######## Test 3 #######')
        print(dataList[CPU][3])
        print(' ######## Maksimali paklaida viso Algoritmo metu #######')
        print(dataList[CPU][4])
