...
class Sprendimas:
    ...
    def CalculatePointsXY_simple(self, t0, tN, tkList, N, rez, fmin, step):
        ...
        for j in range(1, rez+1): # j - zinsniu kiekis, kiek h reikia padaryti fmin -> fmax; Ar reikia imtinai rez (rez+1)??
    ...
    def CalculatePointsXY_multithreading(self, t0, tN, tkList, N, rez, fmin, step):
        intervalsList = self.threads_intervalList #list[[]]
        queuePoints = self.queuePoints
        if (self.fmax != 1): rez += 1 # !@@@ Jei fmax == 10**0, ty bus fj==1, iskreips grafika
        sveikojiDalis =  int(rez / self.threads_q)
        liekana = rez % self.threads_q
        begin=0
        end = sveikojiDalis
